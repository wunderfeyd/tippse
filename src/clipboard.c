...
#elif __linux__
  clipboard_command_set(external_data, external_encoding, "((xclip -i --selection primary) || (xsel -i -p) || (wl-copy -p)) 2>/dev/null");
  clipboard_command_set(external_data, external_encoding, "((xclip -i --selection clipboard) || (xsel -i -b) || (wl-copy)) 2>/dev/null");
#elif _WINDOWS
...
#elif __linux__
  data = clipboard_command_get(encoding, "((xclip -o --selection clipboard) || (xsel -o -b) || (wl-paste -n)) 2>/dev/null");
  /*if (!data) {
