...
void file_type_markdown_mark(struct document_text_render_info* render_info) {
  ...
  if (before&VISUAL_DETAIL_NEWLINE) {
    before &= ~(VISUAL_DETAIL_STRING0|VISUAL_DETAIL_STRING1|VISUAL_DETAIL_STRING2);
  }
  ...
  } else if ((cp1=='*' || cp1=='-') && (cp2==' ' || cp2=='\t') && !(before&VISUAL_DETAIL_STOPPED_INDENTATION)) {
  ...
  } else if (before&VISUAL_DETAIL_STRING0) {
