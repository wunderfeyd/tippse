...
struct range_tree* search_replacement(struct search* base, struct range_tree* replacement_tree, struct encoding* replacement_encoding, struct range_tree* document_tree) {
  ...
  stream_from_page(&replacement_stream, range_tree_first(replacement_tree), 0);
  ...
  while (1) {
    ...
    if (size>TREE_BLOCK_LENGTH_MIN-8 || cp<=0) {
      range_tree_insert_split(output, range_tree_length(output), &coded[0], size, 0);
      size = 0;
    }
      ...
      if (cp=='r') {
        ...
        if (group<base->groups) {
          ...
          if (start<end) {
            ...
            range_tree_paste(output, copy->root, range_tree_length(output));
...
size_t search_append_set(struct search_node* last, int ignore_case, struct unicode_sequencer* sequencer, size_t offset) {
  ...
  if (ignore_case) {
    ...
    struct range_tree* source = range_tree_copy(&check->set, 0, range_tree_length(&check->set), NULL);
...
TIPPSE_INLINE int search_node_set_check(struct search_node* node, codepoint_t index) {
  return range_tree_node_marked(node->set.root, (file_offset_t)index, 1, TIPPSE_INSERTER_MARK);
}
